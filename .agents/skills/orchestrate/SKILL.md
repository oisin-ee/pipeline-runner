---
description: Local multi-agent orchestrator — decompose a task and fan it out to the MoKa specialist roster on the current machine instead of submitting to the remote Moka pipeline. The local twin of the MoKa Orchestrator. On Claude Code, choose either the agents configured in Claude Code or the agents configured in OpenCode; on OpenCode, use OpenCode's native subagents. Use when the user wants parallel specialist agents driven locally rather than as Argo/k8s jobs.
name: orchestrate
---
...
The same specialist agents the MoKa pipeline uses, mirrored locally. The OpenCode roster is `mode: all`, so it works both as an `opencode run --agent` subprocess and as a native OpenCode subagent. When choosing Claude Code agents, use the matching Claude Code roster for the same roles.
...
The orchestration **doctrine below is identical whichever local agent pool you choose**. Only the spawn mechanism differs — select the agent pool you are actually using for this run, and do not mix pools within one lane.

### Claude Code agents

Use the MoKa roster configured in Claude Code when that is the local agent pool you choose for the run. Dispatch the matching Claude Code agent for each lane, using the host's configured Claude Code agent-dispatch mechanism.

You should use Haiku, Sonnet, Opus, and Fable agents when the task calls for a specific specialization, and these agents should be used to specifically reduce the cost of agent usage. For example, a mechanical task could use Haiku or Sonnet, while a high-thinking task like architecture or debugging could use Opus or Fable.

- Use the same role/name from the roster table above, or the local Claude Code equivalent when the names differ.
- Parallelize independent lanes through Claude Code's agent mechanism; sequence dependent lanes.
- Pass context by path, not by paste — agents read the worktree directly. Hand them the files/AC the Researcher produced.
- Keep the same lane write boundaries and expected artifacts. The choice of agent pool must not change what a lane is allowed to write or what it must return.

### OpenCode agents from Claude Code → `opencode run`
...
- **Parallelize independent lanes**: launch each `opencode run` as a background shell process, then collect. Run dependent lanes only after their inputs land.
- Pass context by path, not by paste — agents read the worktree directly. Hand them the files/AC the Researcher produced.
- `--model` / `--variant` only when a lane genuinely needs a different tier; otherwise inherit.

### OpenCode native agents

When already inside OpenCode, use OpenCode's native subagent mechanism instead of shelling out to `opencode run`. Spawn the roster directly, selecting the agent by the same name:
...
Whichever local agent pool you choose, run the same six steps:
...
2. **Dispatch** — Fan out per the dispatch option above. Scope each agent tightly: one job, its lane's write boundary, explicit acceptance criteria.
...
- **Doctrine is agent-pool-neutral; only the Dispatch section is pool-specific.** Do not leak `opencode run` syntax into a Claude Code agent run, or Claude Code agent instructions into an OpenCode run.
...
Orchestrate is `moka submit` brought home: same roster, same decompose → dispatch → gather → gate → learn → synthesize loop, run on this machine. Choose the local agent pool for the run: Claude Code agents, OpenCode agents via `opencode run`, or OpenCode native agents when already inside OpenCode. You stay the orchestrator — fan out the lanes, gate on real verifier *and* reviewer evidence, capture lessons via the Learner, and report only what the agents proved.
