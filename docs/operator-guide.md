...
Installs or refreshes Moka-owned host adapters in one command: generated
host-native command surfaces, native-agent projections, and the singleton
`pipeline-gateway` MCP host config. OpenCode is the package default runtime.
Adapters are installed globally (`~/.claude`, `~/.config/opencode`, `~/.codex`);
there is no `--scope`. `moka init` does not create repo-local `.pipeline`
config files.

```shell
moka init             # install or refresh Moka host adapters
moka init --check     # verify generated Moka host adapters are current; fail if stale
moka init --dry-run   # show planned changes without writing
moka init --force     # overwrite manually edited adapter files
```

`--check` and `--dry-run` write nothing. By default `moka init` refuses to
overwrite manually edited adapter files; `--force` overwrites them. The shared
agent harness (skills, hooks, instruction rules, and Claude settings) is
installed from `oisin-ee/agent` by chezmoi, not by Moka.
...
Submit settings currently include service account, image pull secret, CPU/memory
requests and limits, event sink URL, broker auth, DB auth, MCP gateway auth,
event auth Secret/key, and Git/GitHub/npm auth Secret refs. There is no queue
name setting. Workflow-level active deadline and TTL become caller-facing submit
settings only when PIPE-99 threads them through `moka submit`. The runner-side
event auth token is mounted as a file via Secret volume at the path configured
in `events.authTokenFile`.
...
Shared skill and hook files are installed from `oisin-ee/agent` by chezmoi, not
generated by Moka.
...
server into host config. For OpenCode, existing plugin entries are preserved
while missing package defaults such as telemetry and goal-context plugins are
