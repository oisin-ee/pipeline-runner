...
- The configured broker API key Secret exposed to the runner as `BROKER_API_KEY`
  from `brokerAuth.secretName[brokerAuth.secretKey]`; `BROKER_URL` is literal
  config
...
The runner image sets `HOME=/root`; OpenCode auth is generated inside the pod
from the broker env before the session starts. The runner does not read event
auth tokens or payloads from environment variables. No
`OISIN_PIPELINE_RUNNER_PAYLOAD_JSON`, `PIPELINE_EVENT_API_TOKEN`,
`OPENCODE_AUTH_JSON`, or mounted OpenCode `auth.json` is used.
