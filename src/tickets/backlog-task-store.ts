...
import { mutableArray, parseResultWithSchema, trimmedRequiredString, withDefault, struct } from "../schema-boundary";
...
export class BacklogTaskStoreError extends Data.TaggedError("BacklogTaskStoreError")<{
  readonly message: string;
  readonly path?: string;
}> {}

const extractMarkedBlock = (content: string, markers: { readonly end: string; readonly start: string }): string => {
...
const extractDescription = (content: string): string => extractMarkedBlock(content, DESCRIPTION_MARKERS);
...
const extractAcceptanceCriteria = (content: string): string[] =>
  extractMarkedBlock(content, ACCEPTANCE_MARKERS)
    ...
    .filter((criterion): criterion is string => criterion !== undefined && criterion.length > 0);
...
const decodeFrontmatterEffect = (
  ...
  return Effect.fail(
    storeError(filePath, `Invalid Backlog task frontmatter in ${filePath}: ${formatSchemaIssues(decoded.issues)}`),
  );
};
...
): Effect.Effect<ReadonlyMap<string, BacklogTaskRecord>, BacklogTaskStoreError> => {
  const tasksById = new Map<string, BacklogTaskRecord>();
  for (const task of tasks) {
    const existing = tasksById.get(task.id);
    if (existing) {
      return Effect.fail(
        storeError(task.filePath, `Duplicate Backlog task id ${task.id}: ${existing.filePath} and ${task.filePath}`),
      );
    }
...
const readBacklogTasksEffect = (
  ...
  return Effect.gen(function* effectBody() {
    ...
    const exists = yield* repoIo.exists(tasksDir).pipe(Effect.mapError((error) => ioStoreError(tasksDir, error)));
    if (!exists) {
      return [];
    }
      ...
      entries
        ...
        .map((entry) => {
          ...
          return repoIo.readText(filePath).pipe(
            ...
            Effect.flatMap((source) => parseBacklogTaskMarkdownEffect(source, filePath)),
          );
        }),
    );
  });
