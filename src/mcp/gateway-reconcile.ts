...
> => {
  ...
  return Effect.gen(function* effectBody() {
    ...
    const workspacePath =
      env.PIPELINE_TARGET_PATH !== undefined && env.PIPELINE_TARGET_PATH !== ""
        ? env.PIPELINE_TARGET_PATH
        : cwd;
...
export const startLocalGateway = async (
  config: PipelineConfig,
  cwd: string
): Promise<void> => {
  await runMcpGatewayEffect(
    Effect.gen(function* effectBody() {
      ...
      const service = yield* McpGatewayService;
      return yield* service.serveToolHiveVmcp(result.configPath, cwd);
    })
  );
};
...
export const reconcileGateway = async (
  ...
  await runMcpGatewayEffect(reconcileGatewayEffect(config, cwd, env));
