...
export type ClaudeUserConfigMergeResult = { content: string; ok: true } | { errors: ParseError[]; ok: false };

const mergeClaudeUserConfigWith = (
  projection: ClaudeUserConfigProjection,
  project: (currentText: string, parsed: Record<string, unknown>, projection: ClaudeUserConfigProjection) => string,
  currentText = "",
...
export const replaceClaudeUserMcpServers = (
  ...
  mergeClaudeUserConfigWith(
    projection,
    (text, _parsed, value) => applyJsonEdit(text, ["mcpServers"], value.mcpServers ?? {}),
    currentText,
  );
