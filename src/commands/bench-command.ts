...
import { buildEvalReport, parseEvalRunResultsJson, renderEvalReport } from "../bench/eval-report";
import type { EvalRunResult } from "../bench/eval-report";

class BenchCommandService extends Context.Service<
  BenchCommandService,
  {
    readonly readResults: (path: string) => Effect.Effect<EvalRunResult[], unknown>;
...
const BenchCommandServiceLive = Layer.succeed(BenchCommandService, {
  readResults: (path) => Effect.try(() => parseEvalRunResultsJson(readFileSync(path, "utf-8"))),
...
export const registerBenchCommand = (program: Command): void => {
  program
    .command("bench")
    .description("Score a flat single-agent baseline vs the pipeline over recorded bench runs")
    .requiredOption("--results <path>", "JSON file: array of { task, variant, resolved, costTokens, wallMs }")
    .action(async (options: { results: string }) => {
      await Effect.runPromise(Effect.provide(runBenchCommand(options), BenchCommandServiceLive));
    });
};
