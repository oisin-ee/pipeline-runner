...
import type { BacklogTaskRecord, BacklogTaskStoreError } from "../../tickets/backlog-task-store";
import { buildTicketGraphEffect, scopedTicketIds } from "../../tickets/ticket-graph";
import type { TicketGraph, TicketGraphError } from "../../tickets/ticket-graph";
import type { TicketSelectionOptions, TicketSelectionStrategy } from "../../tickets/ticket-selection";

export type TicketPlanExecutor = (plan: RunnerLaunchPlan, options: RunnerExecutionOptions) => Promise<AgentResult>;
...
export const currentWorktreePath = (): string => process.env.PIPELINE_TARGET_PATH ?? process.cwd();
...
export const formatNextTicket = (ticket: Option.Option<BacklogTaskRecord>): string =>
  Option.match(ticket, {
...
export const readyTicketEffect = (
  ...
  Option.match(ticket, {
    onNone: () => Effect.fail(new TicketCommandError({ message: "No ready tickets." })),
...
export const claimTicketEffect = (
  ...
  Effect.gen(function* effectBody() {
    ...
    yield* backlog.run(["task", "edit", ticket.id, "--status", "In Progress", "--plain"], worktreePath).pipe(
      Effect.mapError(
        (error) =>
...
): Effect.Effect<LoadedTicketGraph, BacklogTaskStoreError | TicketGraphError | TicketCommandError, RepoIoService> =>
  Effect.gen(function* effectBody() {
...
const isTicketSelectionStrategy = (strategy: string): strategy is TicketSelectionStrategy =>
  TICKET_SELECTION_STRATEGY_NAMES.has(strategy);

const parseSelectionStrategyEffect = (
  strategy: Option.Option<string>,
): Effect.Effect<Option.Option<TicketSelectionStrategy>, TicketCommandError> => {
...
> =>
  Effect.gen(function* effectBody() {
    ...
    const strategy = yield* parseSelectionStrategyEffect(Option.fromUndefinedOr(flags.strategy));
...
export const runTicketProgram = async <A, E>(program: Effect.Effect<A, E, RepoIoService>) =>
...
) => await Effect.runPromise(Effect.provide(Effect.provide(program, RepoIoServiceLive), backlogLayer));
