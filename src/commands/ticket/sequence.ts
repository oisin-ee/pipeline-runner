...
const formatSequence = (batches: readonly (readonly string[])[]): string =>
  batches.map((batch, index) => [`Sequence ${index + 1}:`, ...batch.map((id) => `  ${id}`)].join("\n")).join("\n\n");

const printTicketSequenceEffect = (worktreePath: string, flags: TicketSequenceFlags) =>
...
export const registerSequenceSubcommand = (ticketCommand: Command, _options: TicketCommandOptions): void => {
  ticketCommand
    .command("sequence")
    ...
    .action(async (flags: TicketSequenceFlags) => {
      await runTicketProgram(printTicketSequenceEffect(currentWorktreePath(), flags));
    });
};
