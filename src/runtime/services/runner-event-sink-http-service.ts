...
const EventSinkHttpError = Data.TaggedError("EventSinkHttpError")<{
  readonly status: number;
  readonly message: string;
}>;
type EventSinkHttpError = InstanceType<typeof EventSinkHttpError>;
...
const toHttpError = (error: unknown): EventSinkHttpError => {
  ...
  return new EventSinkHttpError({
    message: `Event sink responded with ${status}${data ? `: ${data}` : ""}`,
    status,
  });
};
