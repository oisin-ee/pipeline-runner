...
import { OpenPullRequestGitService, OpenPullRequestGitServiceLive } from "./open-pull-request-git-service";
...
describe("OpenPullRequestGitServiceLive", () => {
  it("routes every git op through the authenticated runner (no naked git)", async () => {
    ...
    const program = Effect.gen(function* program() {
      ...
      return yield* git.raw(["push", "--force-with-lease", "origin", "HEAD:refs/heads/moka/run/x"]);
    });

    const output = await Effect.runPromise(Effect.provide(program, OpenPullRequestGitServiceLive));
