...
const checkoutDrainMergeIntegrationBranch = (
  ...
  Effect.gen(function* checkoutDrainMergeIntegrationBranchProgram() {
    const checkedOut = yield* Effect.result(
      git
        .raw(["rev-parse", "--verify", integrationBranch])
        .pipe(Effect.flatMap(() => git.raw(["checkout", integrationBranch])))
    );
    if (Result.isSuccess(checkedOut)) {
      return;
    }
    yield* git.raw(["checkout", "-b", integrationBranch, baseSha]);
  });
...
const drainMergeConflictFiles = (
  ...
  Effect.gen(function* drainMergeConflictFilesProgram() {
    const diff = yield* Effect.result(
      git.raw(["diff", "--name-only", "--diff-filter=U"])
    );
    if (Result.isFailure(diff)) {
      return [];
    }
    return diff.success.split(LINE_RE).filter(Boolean);
  });

const abortDrainMerge = (git: DrainMergeGitClient): Effect.Effect<void> =>
  Effect.result(git.raw(["merge", "--abort"])).pipe(Effect.asVoid);
...
const mergeDrainMergeChild = (
  ...
  Effect.gen(function* mergeDrainMergeChildProgram() {
    const result = yield* Effect.result(
      drainMergeChild(git, child.output.branch)
    );
    if (Result.isFailure(result)) {
      yield* recordDrainMergeConflict(git, report, child);
      return;
    }
...
const drainMergeProgram = (
  ...
  return Effect.gen(function* effectBody() {
    ...
    if (Result.isFailure(setupResult)) {
