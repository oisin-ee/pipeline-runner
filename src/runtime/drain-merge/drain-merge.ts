...
import { fromUndefinedOr, getOrNull, getOrUndefined, isNone, isSome, none, some } from "effect/Option";
...
import { DrainMergeGitService, DrainMergeGitServiceLive } from "../services/drain-merge-git-service";
...
const firstNeededNode = (node?: PlannedWorkflowNode): Option<string> => fromUndefinedOr(node?.needs.at(0));

const runIdForDrainMerge = (context: RuntimeContext): string => context.runId ?? generateRuntimeRunId();

const drainMergeWorktreeOutput = (output: DrainMergeChildOutput): Option<DrainMergeWorktreeOutput> => {
  ...
  if (baseSha === undefined || branch === undefined || worktreePath === undefined) {
    return none();
  }
  return some({
    baseSha,
...
const drainMergeChild = (git: DrainMergeGitClient, branch: string): Effect.Effect<string, unknown> =>
  git.raw(["merge", "--no-ff", "--no-edit", "-m", "drain-merge: merge", branch]);
...
const drainMergeConflictFiles = (git: DrainMergeGitClient): Effect.Effect<string[]> =>
...
const stringFieldOption = (value: unknown): Option<string> => (typeof value === "string" ? some(value) : none());

const drainMergeStatus = (value: unknown): DrainMergeStatus => (value === "PASS" ? "PASS" : "FAIL");

const parseDrainMergeChildOutput = (value: unknown): Option<DrainMergeChildOutput> => {
...
const drainMergeChildren = (
  ...
  const output = parseJsonObject(context.nodeStateStore.lastOutputByNode.get(upstreamNodeId.value));
  ...
  return (upstream.children ?? []).flatMap((child) => {
    ...
    return isSome(childOutput) ? [{ nodeId: child.id, output: childOutput.value }] : [];
  });
};

const hasDrainMergeFailure = (report: DrainMergeReport, failed?: boolean): boolean =>
  report.conflicts.length > 0 || failed === true;

const drainMergeSummary = (report: DrainMergeReport, hasFailure: boolean): string =>
  hasFailure
    ? `drain-merge completed with ${report.conflicts.length} conflicts`
    : `drain-merge merged ${report.merged.length} branches`;

const drainMergeEvidence = (report: DrainMergeReport, hasFailure: boolean, evidence?: string[]): string[] => [
  ...(evidence ?? []),
  drainMergeSummary(report, hasFailure),
];
...
const drainMergeSetupErrorResult = (report: DrainMergeReport, error: unknown): NodeAttemptResult =>
...
export const executeDrainMergeBuiltin = async (
  ...
  await Effect.runPromise(Effect.provide(drainMergeProgram(context, node), DrainMergeGitServiceLive));
