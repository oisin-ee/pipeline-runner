...
describe("raceDetached", () => {
  it("preserves the timeout failure when interrupting a source with a hanging finalizer", async () => {
    ...
    const timeout = Effect.sleep(Duration.millis(20)).pipe(
      Effect.andThen(Effect.fail(new Error("timeout won")))
    );
    const started = Date.now();

    await expect(
      Effect.runPromise(raceDetached(source, timeout))
    ).rejects.toThrow("timeout won");
