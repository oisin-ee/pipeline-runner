...
function writeManifest(path: string, manifest: WorktreeManifest): void {
  writeFileSync(path, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
}

function readManifest(path: string): WorktreeManifest {
  return JSON.parse(readFileSync(path, "utf8")) as WorktreeManifest;
}
...
function retentionState(
  absPath: string,
  baseSha: string
): "retained-dirty" | "retained-unpushed" | undefined {
...
export function gcParallelWorktrees(repoRoot: string): WorktreeState[] {
