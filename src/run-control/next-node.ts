...
import { buildEnvelopeForNode, collectStoredResults } from "../runtime/step/step-node";
...
export type { NextNodeInput, NodeEnvelopeMetadata } from "../runtime/step/step-node";
 ...
 */
const buildNextNodeEnvelopeOption = (input: NextNodeInput): Option.Option<NextNodeEnvelope> => {
...
export const readPersistedScheduleEffect = (store: RunControlStore, runId: string): Effect.Effect<string, unknown> =>
  Effect.gen(function* effectBody() {
    const manifest = yield* store.readRun({ runId });
    if (manifest === undefined) {
      return yield* Effect.fail(new Error(`Run ${runId} does not exist in the Moka DB.`));
    }
...
> =>
  Effect.gen(function* effectBody() {
    const scheduleRaw = yield* readPersistedScheduleEffect(input.runControlStore, input.runId);
    const compiled = yield* Effect.try({
      catch: (error) => error,
      try: () =>
        compileScheduleArtifact(
          input.config,
          parseScheduleArtifact(scheduleRaw, `persisted schedule for ${input.runId}`),
          input.worktreePath,
        ),
    });
    const { plan } = compiled;
...
export const buildNextNodeEnvelopeFromRunStore = (input: NextNodeRunStoreInput) =>
  Effect.gen(function* effectBody() {
...
export const readyNodeIdsFromRunStore = (input: NextNodeRunStoreInput): Effect.Effect<string[], unknown> =>
  Effect.gen(function* effectBody() {
...
const printNextNodeEnvelopeScoped = (runId: string): Effect.Effect<void, unknown, Scope.Scope> =>
  Effect.gen(function* effectBody() {
...
const printNextNodeEnvelopeEffect = (runId: string): Effect.Effect<void, unknown> =>
...
export const registerNextNodeSubcommand = (nextCommand: Command): void => {
  nextCommand
    .command("node")
    .description("Emit the next ready node envelope from a persisted run without executing it")
    .argument("<run-id>", "the run id to query")
    .showHelpAfterError("Remove --schedule-file; moka next node reads schedules from the Moka DB by run id.")
