...
const SCAFFOLD_FILES: Record<string, string> = {
  ...
  ".pipeline/prompts/orchestrator.md": [
    ...
    "Use package-owned `@oisincoveney/pipeline` config as the source of truth for workflow order, profiles, gates, hooks, and artifacts.",
...
function hostResourceInput(host: string): string {
  return [
    ...
    "Package-owned `@oisincoveney/pipeline` config is the source of truth; generated host resources must preserve the profiles, prompts, rules, tools, filesystem policy, network policy, and output contracts declared there.",
    "",
  ].join("\n");
}
