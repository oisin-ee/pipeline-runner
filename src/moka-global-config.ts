...
import {
  throwCauseFailureAsError,
  unknownErrorMessage,
} from "./effect-sync-errors";
import {
  dbAuthOptionSchema,
...
const optionalStringValue = (value: Option.Option<string>): OptionalString =>
  Option.getOrUndefined(value);

const runPathSync = <A, E>(program: Effect.Effect<A, E, Path.Path>): A => {
  const exit = Effect.runSyncExit(
    Effect.provide(
      Effect.provide(program, Path.layer),
...
const mokaGlobalConfigPathEffect = (
  homeDir?: string
): Effect.Effect<string, Config.ConfigError, Path.Path> =>
...
export const mokaGlobalConfigPath = (homeDir?: string): string =>
  runPathSync(mokaGlobalConfigPathEffect(homeDir));
...
const mokaDbUrlReadSchema = struct({
  momokaya: Schema.optional(
    struct({ db: Schema.optional(mokaDbGlobalConfigSchema) })
  ),
});
...
const loadMokaDbUrlEffect = Effect.fn("loadMokaDbUrl")(
  function* loadMokaDbUrlEffect(configPath: string) {
    ...
    if (Option.isSome(envUrl)) {
      ...
      return Option.some(parsed.value.url);
    }
    ...
    if (!parsed.ok) {
      return yield* Effect.fail(
        globalConfigValidationError(
          configPath,
          configIssuesFromSchemaIssues(parsed.issues)
        )
      );
    }
    return Option.fromNullishOr(parsed.value.momokaya?.db?.url);
  }
);
...
export const loadMokaDbUrl = (): OptionalString => {
  ...
  const program = Effect.provide(
    loadMokaDbUrlEffect(configPath),
    ConfigProvider.layer(ConfigProvider.fromEnv())
  );
  return Result.match(runConfigIoSync(Effect.result(program)), {
    onFailure: (error) => {
      ...
      return optionalStringValue(Option.none());
    },
    onSuccess: optionalStringValue,
  });
};

const parseMokaGlobalConfigEffect = Effect.fn("parseMokaGlobalConfig")(
...
export const loadMokaGlobalConfig = () => {
  const configPath = mokaGlobalConfigPath();
  const program = Effect.fn("loadMokaGlobalConfig")(function* program() {
    const configIo = yield* ConfigIoService;
    const source = yield* configIo.readOptionalText(configPath);
