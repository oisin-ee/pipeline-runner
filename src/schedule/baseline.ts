...
const quickBaselineWorkflow = (): ScheduleArtifact["workflows"] => ({
  root: {
...
const executeBaselineWorkflow = (): ScheduleArtifact["workflows"] => ({
  root: {
...
const defaultScheduleId = (date: Date): string =>
  `run-${date
    .toISOString()
    .replaceAll(/[-:.TZ]/gu, "")
    .slice(0, 14)}`;

export const baselineScheduleArtifact = (input: {
  baseline: ScheduleBaseline;
