...
import { isRecord, parseJson, recordKeys } from "../src/safe-json";
...
const PUBLIC_RUNTIME_CONTRACT_SET = HashSet.make(...PUBLIC_RUNTIME_CONTRACTS);
...
const RUNTIME_OBSERVABILITY_INSPECTION_IMPORT_RE = /from\s+["'][^"']*runtime-observability-inspection["']/u;
const RUNTIME_MACHINE_IMPORT_RE = /from\s+["'][^"']*runtime-machines\//u;
const GATE_OR_HOOK_MACHINE_IMPORT_RE = /from\s+["'][^"']*runtime-machines\/(?:gate-machine|hook-machine)["']/u;
const NODE_MACHINE_IMPORT_RE = /from\s+["'][^"']*runtime-machines\/node-machine["']/u;
...
const RUNTIME_CONTEXT_INTERFACE_RE = /export interface RuntimeContext \{[\s\S]*?\n\}/u;
const RUNTIME_MACHINES_CONTRACTS_TEXT_RE = /runtime-machines\/contracts/u;
const RUNTIME_ACTOR_DESCRIPTOR_EXPORT_RE = /export\s+(?:type|interface)\s+RuntimeActorDescriptor\b/u;
const RUNTIME_MACHINES_PATH_SEGMENT = "runtime-machines";
const PATH_SEPARATOR_RE = /[\\/]/u;
const TYPE_KEYWORD_RE = /\btype\b/gu;
const IMPORT_ALIAS_RE = /\s+as\s+/u;
const IGNORED_SCAN_DIRS = HashSet.make(".git", ".fallow", "coverage", "dist", "node_modules");

class RuntimeActorContractBoundaryTestError extends taggedErrorClass<RuntimeActorContractBoundaryTestError>()(
  "RuntimeActorContractBoundaryTestError",
  {
    message: stringValue(),
  },
) {}

const runtimeActorContractBoundaryTestError = (message: string): RuntimeActorContractBoundaryTestError =>
  new RuntimeActorContractBoundaryTestError({ message });
...
const repositoryFiles = (dir: string): string[] =>
  readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    if (HashSet.has(IGNORED_SCAN_DIRS, entry.name)) {
      return [];
    }
    ...
    return [".ts", ".tsx", ".mts", ".cts"].includes(extname(entry.name)) ? [absolute] : [];
  });
  ...
  return [...imports]
    .flatMap((match) => {
      ...
      if (names) {
        return names
          .split(",")
          .map((name) => name.replace(TYPE_KEYWORD_RE, "").trim().split(IMPORT_ALIAS_RE)[0])
          .filter(Boolean);
      }

      return defaultName ? [defaultName] : [];
    })
    .filter((name) => HashSet.has(PUBLIC_RUNTIME_CONTRACT_SET, name));
};
...
describe("runtime actor/retry contract module boundary", () => {
  it("removes the runtime-machines TypeScript surface and all runtime-machines imports", () => {
    const runtimeMachineFiles = sourceFiles(join(process.cwd(), "src", "runtime-machines")).map((file) =>
      relative(process.cwd(), file),
    );
    const runtimeMachineNamedFiles = repositoryFiles(process.cwd())
      .map((file) => relative(process.cwd(), file))
      .filter((file) => file.split(PATH_SEPARATOR_RE).some((part) => part.includes(RUNTIME_MACHINES_PATH_SEGMENT)));
    const offenders = repositoryFiles(process.cwd()).flatMap((file) => {
      const source = readFileSync(file, "utf-8");

      return RUNTIME_MACHINE_IMPORT_RE.test(source) ? [{ file: relative(process.cwd(), file) }] : [];
    });
  ...
  it("removes legacy inspection-bridge imports and direct xstate imports from runtime and tests", () => {
    const offenders = repositoryFiles(process.cwd()).flatMap((file) => {
      const source = readFileSync(file, "utf-8");
      const violations = [
        RUNTIME_OBSERVABILITY_INSPECTION_IMPORT_RE.test(source) ? "runtime-observability-inspection" : undefined,
        XSTATE_IMPORT_RE.test(source) ? "xstate" : undefined,
      ].filter(Boolean);

      return violations.length ? [{ file: relative(process.cwd(), file), violations }] : [];
    });

    expect(existsSync(join(process.cwd(), "src/runtime-observability-inspection.ts"))).toBe(false);
    expect(offenders).toEqual([]);
  });

  it("keeps xstate out of package metadata and lockfile", () => {
    const packageJson = parseJson(readFileSync(join(process.cwd(), "package.json"), "utf-8"), "package.json");
    if (!isRecord(packageJson)) {
      throw runtimeActorContractBoundaryTestError("package.json must be an object");
    }
    ...
    expect(dependencySections.flatMap((section) => recordKeys(section))).not.toContain("xstate");
  ...
  it("inlines gate and hook runtime evaluation without xstate machine dependencies", () => {
    const runtimeModules = ["src/runtime/gates/gates.ts", "src/runtime/hooks/hooks.ts"];

    const offenders = runtimeModules.flatMap((file) => {
      const source = readFileSync(join(process.cwd(), file), "utf-8");
      const forbiddenImports = [XSTATE_IMPORT_RE, GATE_OR_HOOK_MACHINE_IMPORT_RE].filter((pattern) =>
        pattern.test(source),
      );
    ...
    expect(offenders).toEqual([]);
    expect(
      ["src/runtime-machines/gate-machine.ts", "src/runtime-machines/hook-machine.ts"].filter((file) =>
  ...
  it("uses a plain node state tracker instead of the deleted node xstate machine", () => {
    const nodeMachinePath = join(process.cwd(), "src/runtime-machines/node-machine.ts");
    const offenders = sourceFiles(join(process.cwd(), "src")).flatMap((file) => {
      const source = readFileSync(file, "utf-8");

      return NODE_MACHINE_IMPORT_RE.test(source) ? [{ file: relative(process.cwd(), file) }] : [];
    });
  ...
  it("does not make retry observability depend on a RETRYING getSnapshot round trip", () => {
    const pipelineRuntime = readFileSync(join(process.cwd(), "src/pipeline-runtime.ts"), "utf-8");
  ...
  it("keeps source files off runtime-machines/contracts for public actor, node event, and retry contracts", () => {
    const offenders = sourceFiles(join(process.cwd(), "src"))
      ...
      .flatMap(({ file, relativeFile }) => {
        const importedContracts = importsFromRuntimeMachineContracts(readFileSync(file, "utf-8"));

        return importedContracts.length ? [{ file: relativeFile, importedContracts }] : [];
      });

    expect(offenders).toEqual([]);
    const trackerSource = readFileSync(join(process.cwd(), "src/runtime/node-state-tracker.ts"), "utf-8");
    ...
    expect(readFileSync(join(process.cwd(), "src/runtime/actor-ids.ts"), "utf-8")).toMatch(
      RUNTIME_ACTOR_DESCRIPTOR_EXPORT_RE,
    );
  });
  ...
  it("does not add or remove package export paths while introducing the internal actor id module", () => {
    ...
    if (!isRecord(packageJson) || !isRecord(packageJson.exports)) {
      throw runtimeActorContractBoundaryTestError("package.json exports must be an object");
    }

    expect(recordKeys(packageJson.exports).toSorted()).toEqual([...EXPECTED_PACKAGE_EXPORTS].toSorted());
  });

  it("does not build retired goal runtime entrypoints as standalone package surfaces", () => {
    const tsdownConfig = readFileSync(join(process.cwd(), "tsdown.config.ts"), "utf-8");
  ...
  it("exposes runtime node execution state through the internal NodeStateStore field only", () => {
    const contractsSource = readFileSync(join(process.cwd(), "src/runtime/contracts/contracts.ts"), "utf-8");
    const runtimeContextSource = RUNTIME_CONTEXT_INTERFACE_RE.exec(contractsSource)?.[0];

    expect(runtimeContextSource).toBeDefined();
    expect(runtimeContextSource).toMatch(NODE_STATE_STORE_FIELD_RE);
    expect(runtimeContextSource).not.toMatch(LEGACY_RUNTIME_CONTEXT_NODE_STATE_FIELDS_RE);
  });
});
