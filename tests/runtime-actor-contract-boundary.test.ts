...
const IGNORED_SCAN_DIRS = HashSet.make(
  ".git",
  ".fallow",
  "coverage",
  "dist",
...
const runtimeActorContractBoundaryTestError = (
  message: string
): RuntimeActorContractBoundaryTestError =>
...
describe("runtime actor/retry contract module boundary", () => {
  ...
  it("keeps xstate out of package metadata and lockfile", () => {
    ...
    if (!isRecord(packageJson)) {
      throw runtimeActorContractBoundaryTestError(
    ...
    expect(
      dependencySections.flatMap((section) => recordKeys(section))
  ...
  it("does not add or remove package export paths while introducing the internal actor id module", () => {
    ...
    if (!isRecord(packageJson) || !isRecord(packageJson.exports)) {
      throw runtimeActorContractBoundaryTestError(
        "package.json exports must be an object"
      );
    }

    expect(recordKeys(packageJson.exports).toSorted()).toEqual(
