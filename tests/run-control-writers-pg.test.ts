...
describePg(
  ...
  () => {
    const dbUrl = PG_URL;
    const suitePrefix = `rcwriters-${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
    ...
    const readBackFromFreshResolution = (id: string, root: string) =>
      Effect.scoped(
    ...
    it.live(
      "reporter persists run/node/session state to PG, not the filesystem (AC1, AC2, AC3)",
      () =>
        Effect.gen(function* reporterPersistsToPg() {
          ...
          const manifest = yield* readBackFromFreshResolution(
            id,
            workspaceRoot
          );
    ...
    it.live(
      "supervisor heartbeat + stall route through the PG store (AC1)",
      () =>
        Effect.gen(function* supervisorRoutesThroughPgStore() {
          ...
          const heartbeats = yield* Effect.tryPromise(
            async () =>
              await admin`
    ...
    it.live(
      "writeNodeArtifact + updateRunController route through the PG store (AC1)",
      () =>
        Effect.gen(function* artifactAndControllerRouteThroughPgStore() {
          ...
          const artifact = yield* store.writeNodeArtifact({
            content: "hello\n",
            name: "stdout.jsonl",
            nodeId: "writer",
            runId: id,
