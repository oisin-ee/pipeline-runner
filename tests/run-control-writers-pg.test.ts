...
describePg("run-control writers route through the db.url seam (live cluster PG)", () => {
  const dbUrl = PG_URL;
  ...
  const readBackFromFreshResolution = async (id: string, root: string) =>
    await Effect.runPromise(
      Effect.scoped(resolveRunControlStore(dbUrl, root).pipe(Effect.flatMap((store) => store.readRun({ runId: id })))),
    );
  ...
  it("writeNodeArtifact + updateRunController route through the PG store (AC1)", async () => {
    ...
    expect(artifact.path).toBe(`moka_run_control/${id}/nodes/writer/stdout.jsonl`);
