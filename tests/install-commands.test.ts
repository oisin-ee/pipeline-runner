...
const claudeUserConfigSchema = z.object({
  mcpServers: z.record(z.string(), mcpServerSchema),
  theme: z.string().optional(),
});

const parseOpencodeConfig = (source: string) =>
  opencodeConfigSchema.parse(parseJson(source, "OpenCode config JSON"));

const parseClaudeSettings = (source: string) =>
  claudeSettingsSchema.parse(parseJson(source, "Claude settings JSON"));

const parseClaudeUserConfig = (source: string) =>
  claudeUserConfigSchema.parse(parseJson(source, "Claude user config JSON"));
...
describe("installCommands", () => {
  ...
  it("keeps install planning and filesystem writing in separate owner modules", () => {
    const missingOwnerFiles = INSTALL_OWNER_FILES.filter(
    ...
    for (const marker of INSTALL_ORCHESTRATOR_WRITE_MARKERS) {
  ...
  it("installs all generated host resources from package defaults", async () => {
    ...
    expect(
      readFileSync(join(dir, ".opencode/commands/moka-execute.md"), "utf-8")
  ...
  it("preserves unowned OpenCode skill files", async () => {
    ...
    expect(
      readFileSync(join(dir, ".opencode/skills/execute/SKILL.md"), "utf-8")
  ...
  it("dry-run reports conflicts without throwing or overwriting", async () => {
    ...
    expect(readFileSync(path, "utf-8")).toBe("manual command");
  });
  ...
  it("installs Claude Code command and agent adapters", async () => {
    ...
    const claudeUser = parseClaudeUserConfig(
      readFileSync(join(dir, ".claude.json"), "utf-8")
    );
