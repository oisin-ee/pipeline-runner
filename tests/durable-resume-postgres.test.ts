...
class DurableResumePostgresTestError extends Schema.TaggedErrorClass<DurableResumePostgresTestError>()(
  "DurableResumePostgresTestError",
  {
    message: Schema.String,
  }
) {}

const durableResumePostgresTestError = (
  cause: unknown
): DurableResumePostgresTestError =>
  new DurableResumePostgresTestError({ message: String(cause) });
...
const recordPassedNode =
  (nodeIds: string[]) =>
  async (nodeId: string): Promise<RuntimeNodeResult> =>
    ...
    Effect.scoped(
      Effect.gen(function* effectBody() {
        ...
        yield* Effect.tryPromise({
          catch: durableResumePostgresTestError,
          try: async () => await run(Option.getOrUndefined(journal)),
        });
      })
    )
  );
...
describe("acquireRunJournal selection (no infra)", () => {
  it.effect(
    "resolves no journal when db.url is absent (byte-identical default)",
    () =>
      Effect.gen(function* effectBody() {
        const journal = yield* Effect.scoped(acquireRunJournal("run-1"));
        expect(Option.isNone(journal)).toBe(true);
      })
  );

  it.effect("resolves no journal when runId is absent", () =>
    Effect.gen(function* effectBody() {
      const journal = yield* Effect.scoped(
        acquireRunJournal(undefined, "postgres://unused")
      );
      expect(Option.isNone(journal)).toBe(true);
    })
  );
...
describePg("durable run-journal cutover (live cluster PG)", () => {
  ...
  it.live(
    "resumes a killed run from Postgres without re-running the finished node (AC1)",
    () =>
      Effect.tryPromise({
        catch: durableResumePostgresTestError,
        try: async () => {
            ...
            async (journal) =>
              await runWorkflowScheduler(
                schedulerInput({
                  ...
                  runNode: recordPassedNode(firstRan),
                })
              )
          );
          ...
          await runWithRunJournal(id, dbUrl, async (journal) => {
            resumed = await runWorkflowScheduler(
              schedulerInput({
                journal,
                nodes: [scheduleNode("a", 0), scheduleNode("b", 1, ["a"])],
                runNode: recordPassedNode(resumedRan),
              })
            );
          });
  ...
  it.live(
    "isolates resume by runId — concurrent runs each replay only their own records (AC4)",
    () =>
      Effect.tryPromise({
        catch: durableResumePostgresTestError,
        try: async () => {
              ...
              async (journal) =>
                await runWorkflowScheduler(
                  schedulerInput({
                    ...
                    journal,
                    nodes: [scheduleNode("a", 0), scheduleNode("b", 1, ["a"])],
                    runNode: recordPassedNode(firstA),
                  })
                )
            ),
            runWithRunJournal(
              ...
              async (journal) =>
                await runWorkflowScheduler(
                  schedulerInput({
                    ...
                    runNode: recordPassedNode(firstB),
                  })
                )
            ),
          ]);
          ...
          await runWithRunJournal(idA, dbUrl, async (journal) => {
            outcomeA = await runWorkflowScheduler(
              schedulerInput({
                journal,
                nodes: [scheduleNode("a", 0), scheduleNode("b", 1, ["a"])],
                runNode: recordPassedNode(resumedA),
              })
            );
          });
            ...
            async (journal) =>
              await runWorkflowScheduler(
                schedulerInput({
                  journal,
                  nodes: [scheduleNode("p", 0), scheduleNode("q", 1, ["p"])],
                  runNode: recordPassedNode(resumedB),
                })
              )
          );
