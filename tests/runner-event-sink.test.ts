...
const asTestEffect = (run: () => Promise<void>): Effect.Effect<void, unknown> =>
  Effect.tryPromise({
    catch: (error) => error,
    try: run,
  });
...
describe("runner event sink", () => {
  it.effect(
    "uses injected fetch, assigns integer sequences from 1, batches, adds auth, and supplies timestamps",
    () =>
      asTestEffect(async () => {
  ...
  it.effect(
    "retries retryable HTTP and network failures before succeeding",
    () =>
      asTestEffect(async () => {
        const { createRunnerEventSink } = await loadSinkModule();
        const fetchMock = vi
  ...
  it.effect.each([408, 429, 500, 503, 511])(
  ...
  it.effect("does not retry permanent 400 responses", () =>
    asTestEffect(async () => {
      const { createRunnerEventSink } = await loadSinkModule();
  ...
  it.effect("retries request timeouts", () =>
    asTestEffect(async () => {
      vi.useFakeTimers();
      try {
        const { createRunnerEventSink } = await loadSinkModule();
        const pendingResponse = Promise.withResolvers<Response>().promise;
        ...
        const flush = expect(sink.flush()).rejects.toThrow(
          REQUEST_TIMED_OUT_RE
        );
  ...
  it.effect("flushes batches in the same order events were recorded", () =>
    asTestEffect(async () => {
      const { createRunnerEventSink } = await loadSinkModule();
      const fetchMock = vi.fn(() => okResponse());
      const sink = createRunnerEventSink({
  ...
  it.effect(
    "keeps a failed mid-run batch queued and delivers later records after it",
    () =>
      asTestEffect(async () => {
        const { createRunnerEventSink } = await loadSinkModule();
        ...
        expect(
          deliveredBatches.flatMap((batch) =>
  ...
  it.effect(
    "flushes runtime events without waiting for an explicit final flush",
    () =>
      asTestEffect(async () => {
        const { createRunnerEventSink } = await loadSinkModule();
        const fetchMock = vi.fn(() => okResponse());
  ...
  it.effect(
    "maps runtime events to the top-level fields consumed by console",
    () =>
      asTestEffect(async () => {
        const { mapRuntimeEventToRunnerEventRecords } =
  ...
  it.effect("maps node.session to no wire records instead of throwing", () =>
    asTestEffect(async () => {
      const { mapRuntimeEventToRunnerEventRecords } =
        await loadContractModule();
  ...
  it.effect("records cancellation before the final flush", () =>
    asTestEffect(async () => {
      const { createRunnerEventSink } = await loadSinkModule();
      const fetchMock = vi.fn(() => okResponse());
      const sink = createRunnerEventSink({
  ...
  it.effect(
    "serializes runtime observability events without raw inspection payloads",
    () =>
      asTestEffect(async () => {
        const { createRunnerEventSink } = await loadSinkModule();
        const fetchMock = vi.fn(() => okResponse());
        ...
        sink.recordRuntimeEvent({
          ...
          nodeId: "red",
          summary:
            "node actor pipeline.node.run-123.default.red snapshot recorded",
  ...
  it.effect(
    "flushes a runtime CANCELLED final result and preserves it after a failed flush",
    () =>
      asTestEffect(async () => {
        const { createRunnerEventSink } = await loadSinkModule();
        const fetchMock = vi
