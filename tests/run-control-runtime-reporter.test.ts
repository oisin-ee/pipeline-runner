...
import { isUnknownRecord } from "../src/schema-boundary";
...
describe("run-control runtime reporter bridge", () => {
  ...
  it.live(
    "serializes run-state persistence against a builtin hide window so the run survives parallel mechanical-checks",
    () =>
      Effect.gen(function* hideWindowSurvives() {
        const { acquireRunStateLock } = yield* Effect.tryPromise(
          async () => await import("../src/run-control/run-state-lock")
        );
        ...
        const release = yield* Effect.tryPromise(
          async () => await acquireRunStateLock()
        );
        ...
        expect(
          readJson(runPath(workspaceRoot, runId, "status.json"))
        ).toMatchObject({
          status: "starting",
        });
      })
  ...
  it.live(
    "keeps the run alive when an internal sub-invocation reports a session for an undeclared node",
    () =>
      Effect.tryPromise(async () => {
        ...
        await expect(bridge.flush()).resolves.toBeUndefined();
        const status = readJson(runPath(workspaceRoot, runId, "status.json"));
        expect(status).toMatchObject({
          nodes: { writer: { sessionId: "ses_writer" } },
        });
        const nodes = isUnknownRecord(status) ? status.nodes : undefined;
        expect(
          isUnknownRecord(nodes) && Object.hasOwn(nodes, "writer:handoff")
        ).toBe(false);
      })
  );

  it.live(
    "documents the hazard: a direct run-state write during a hide window fails",
    () =>
      Effect.tryPromise(async () => {
  ...
  it.live(
    "forwards PipelineRuntimeEvents and persists the exact runtime stream",
    () =>
      Effect.tryPromise(async () => {
  ...
  it.live(
    "projects workflow, node, agent, gate, and hook events into run store statuses",
    () =>
      Effect.tryPromise(async () => {
  ...
  it.live("appends node output chunks to the node stdout artifact", () =>
    Effect.tryPromise(async () => {
  ...
  it.live(
    "persists OpenCode session ids as per-node metadata in run status",
    () =>
      Effect.tryPromise(async () => {
        ...
        const sessionEvent: PipelineRuntimeEvent = {
          nodeId: "writer",
          sessionId: "ses_writer",
          type: "node.session",
        };
