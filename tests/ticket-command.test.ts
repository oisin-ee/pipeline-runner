...
type TicketPlanExecutor = NonNullable<
  TicketCommandOptions["ticketPlanExecutor"]
>;
...
const applyOneChildTicketPlanExecutor: TicketPlanExecutor = async () =>
...
describe("moka ticket read-only commands", () => {
  ...
  describe("ticket start", () => {
    ...
    it("claims the selected ticket before dispatching a default local normal write run", async () => {
      ...
      const [[runCall]] = runCommand.mock.calls;
    ...
    it.each([
      ...
      const [backlogCall] = backlogCalls;
      expect(backlogCall.args).toEqual([
        "task",
        "edit",
        "PIPE-84.2",
        "--status",
  ...
  it("renders ticket create dry-run output from a validated scoper result without mutating Backlog", async () => {
    ...
    const ticketPlanExecutor: TicketPlanExecutor = async (plan) => {
  ...
  it("applies a validated ticket plan through BacklogService with an existing parent", async () => {
    ...
    await parseTicketCommandWithOptions(
      ...
      {
        backlogLayer,
        ticketPlanExecutor: applyOneChildTicketPlanExecutor,
      }
    );
