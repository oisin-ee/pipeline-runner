...
import { buildNextNodeEnvelope, registerNextNodeSubcommand } from "../src/run-control/next-node";
...
import { recordSubmitResult, registerSubmitResultSubcommand } from "../src/run-control/submit-result";
...
vi.mock("../src/moka-global-config", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../src/moka-global-config")>();
  return {
    ...actual,
...
const withStore = async <A>(dbUrl: string | undefined, runId: string, use: (store: DurableRunStore) => A): Promise<A> =>
...
const runNextNodeCommand = async (runId: string, worktreePath: string): Promise<string> => {
...
const runSubmitResultCommand = async (runId: string, nodeId: string, result: RuntimeNodeResult): Promise<void> => {
  await captureStdout(
    async () =>
      await submitResultProgram().parseAsync(["submit-result", runId, nodeId, "--json", JSON.stringify(result)], {
        from: "user",
      }),
  );
};
...
describe("resolveDurableStore selection (no infra)", () => {
  it("fails fast when db.url is absent instead of selecting the in-memory store", async () => {
    await expect(Effect.runPromise(Effect.scoped(resolveDurableStore(undefined, "run-1")))).rejects.toThrow(
      DB_URL_REQUIRED_RE,
    );
  });
});

describePg("next-node ⇆ submit-result cross-process stepping (live cluster PG)", () => {
  const dbUrl = PG_URL;
  ...
  it("submit-result persists across a process boundary so a fresh next-node advances (AC2)", async () => {
    ...
    expect(second?.upstreamOutputs).toEqual([{ nodeId: "plan", output: "output of plan" }]);
  ...
  it("command actions read manifest.schedule from the DB and advance across process boundaries (AC1, AC3)", async () => {
    ...
    try {
      ...
      expect(JSON.parse(await runNextNodeCommand(id, worktreePath))).toMatchObject({
        nodeId: "plan",
