...
Refactor the current CLI-centric pipeline into a reusable pipeline primitive that can be invoked either from slash commands inside Codex, Codex, OpenCode, or Pi using the host interface's native subagent/session capabilities, or from a shell CLI using subprocess harness commands. The pipeline behavior should stay the same, but agent execution and phase reporting must become injectable adapters rather than hardwired CLI subprocess behavior.
...
- [x] #3 A slash-command adapter contract is documented for Codex, Codex, OpenCode, and Pi, including how each host supplies task input, target path, agent execution, and phase reporting.
