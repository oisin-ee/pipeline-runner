...
Keep epic To Do until 51.8 lands. Note: several ticket file/symbol references are stale after the moka refactor + config split + gateway.ts split (5732c6e) — corrected per-subtask. gateway.ts → src/mcp/gateway-{config,reconcile,doctor,runtime}.ts; config.ts → src/config/*; runner-job/* removed.
<!-- SECTION:NOTES:END -->
